...
imgSwitch.prototype = {
	...
	setMask: function(bigImg, col, row) {
		var a, b, w = Math.floor(this.Width / col),
			h = Math.floor(this.Height / row),
			l = this.ranges.length;
		this.maskWidth = w;
		this.maskHeight = h;
	...
	timeLine: function(play, end, len) {
		...
		t = 0;
		c = function(f, t) {
			return +f + (t - f) * (s > 1 ? s - Math.floor(s) == 0 ? 1 : s - Math.floor(s) : s)
		};
		var exit;
		if(this.index < 14) {
			exit = 1
		...
		return th = setInterval(function() {
			if(this.index < 14) {
				play(c, s = t++/len)};if(s==exit){end(clearInterval(th))}else{play(c,s=t++/len)
			}
		}, this.Speed)
	},
	fxs: [function(el, x) {
		this.clipDiv(el, x(this.maskHeight, 0), x(0, this.maskWidth), x(0, this.maskHeight), x(this.maskWidth, 0))
	}, function(el, x) {
		this.clipDiv(el, -1, x(0, this.maskWidth), -1, x(this.maskHeight, 0))
	}, function(el, x) {
		this.clipDiv(el, x(this.maskHeight, 0), -1, x(0, this.maskHeight), -1)
	}, function(el, x) {
		this.clipDiv(el, -1, -1, x(0, this.maskHeight), -1)
	}, function(el, x) {
		this.clipDiv(el, -1, x(0, this.maskWidth), -1, -1)
	}, function(el, x) {
		this.clipDiv(el, -1, x(0, this.maskWidth), x(0, this.maskHeight), -1)
	}, function(el, x) {
		this.clipDiv(el, x(this.maskHeight, 0), -1, -1, x(this.maskWidth, 0))
	}],
	clipDiv: function(el, y, w, h, x) {
		this.flag = false;
		var params = [y, w, h, x];
		for(var i = params.length; i--;) {
			params[i] = params[i] < 0 ? 'auto' : params[i] + 'px'
		};
	...
	start: function() {
		...
		var cur = self.arrImgs[self.pos % self.len].src;
		...
		self.timer = self.timeLine(function(x) {
			if(self.index < 14) {
			...
			if(self.Auto) {
				self.pos++;
				self.nextTimer = setTimeout(self.start, self.Pause)
			}
		});
...
window.onload=function(){
	new imgSwitch("imgContainer",{Type:12,Width:840,Height:356,Pause:3000,Speed:"fast",Auto:true,Navigate:"numberic",NavigatePlace:"outer",PicturePosition:"left"})	
}